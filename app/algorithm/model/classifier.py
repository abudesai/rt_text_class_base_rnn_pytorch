...
class Net(Module):
    def __init__(self, rnn_unit, V, T, K, D, M):
        ...
        self.rnn_layer = rnn(
            input_size=self.D, hidden_size=self.M, bidirectional=True
        )  # because GRU and LSTM have a bidirectional argument
...
class Classifier:
    def __init__(
        ...
        **kwargs,
        ...
        self.net = Net(
            rnn_unit=self.rnn_unit, V=self.V, T=self.T, K=self.K, D=self.D, M=self.M
        )
